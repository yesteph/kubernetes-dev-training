...
resource "google_compute_instance" "training-instance" {
  ...
  service_account {
    email  = google_service_account.k8s_cnd.email
    scopes = ["cloud-platform"]
  }

  allow_stopping_for_update = true

  metadata_startup_script = "curl -s https://raw.githubusercontent.com/WeScale/kubernetes-dev-training/main/kubernetes-ressources/terraform/modules/bootstrap-vm.sh | bash -s ${count.index}"
}
